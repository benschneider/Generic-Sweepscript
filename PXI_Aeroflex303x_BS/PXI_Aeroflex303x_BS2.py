...
__version__ = "0.9.7"
# 2014-02-10    0.1.0   Philip Krantz   Added   LO Reference Mode
# 2014-02-20    0.5.0   Philip Krantz & Andreas Bengtsson   Added   External trigger and averaging
# 2014-03-08    0.6.0   Andreas Bengtsson   Added   Return pure I and Q vectors for histograms
# 2014-05-08    0.8.0   Andreas Bengtsson & Philip Krantz   A lot of new stuff added, e.g. folding, histograms, cut out a part of the trace and clean up in general
# 2014-06-11    0.9.0   Andreas Bengtsson   Level correction implemented correctly
# 2014-06-27    0.9.1   Andreas Bengtsson   Removed the option to return the power as you can change to power in the logbrowser
# 2014-06-29    0.9.2   Andreas Bengtsson   Cleaned up
# 2014-07-29    0.9.3   Andreas Bengtsson   A logical error made so the trigger wasn't armed for the last trigger, unclear why it stilled worked though...
# 2014-08-15    0.9.4   Andreas Bengtsson   Added an option to return the trace in units of power in linear scale (for shot thermometer)
# 2014-11-21    0.9.5   Andreas Bengtsson   Removed the option to take the average value and instead added so you can return the trace <I^2> and <Q^2>
# 2015-02-06    0.9.6   Andreas Bengtsson   When the frequency changes, the already stored traces gets reset.
# 2015-09-07	 0.9.7   Ben Schneider       level correction is now before averaging
# 2015-09-07	         Ben Schneider       Magnitude is <|M|> = <(I^2 + Q^2)^0.5>
#                                           instead of (<I>^2 + <Q>^2)^0.5 sim. for phase
#                                           Raise Digitizer Overload exception after 3x succession
# 2015-09-14    0.9.7   Ben Schneider       Digitizer outputs are in V over 1 Ohm
...
class Driver(InstrumentDriver.InstrumentWorker):
    ...
    def performOpen(self, options={}):
        ...
        try:
            # open connection
            self.digitizer = afDigitizerWrapper_BS.afDigitizer_BS()
        ...
        except afDigitizerWrapper_BS.Error as e:
    ...
    def sampleAndAverage(self):
        ...
        for i in range(0, self.nTriggers):
            ...
            # Fold the data
            reshapedI = np.array(lI).reshape(nAvgPerTrigger, self.nSamples+nPreTriggers)[:,range(self.nStartSample-1, self.nStopSample)]/np.sqrt(1000)
            reshapedQ = np.array(lQ).reshape(nAvgPerTrigger, self.nSamples+nPreTriggers)[:,range(self.nStartSample-1, self.nStopSample)]/np.sqrt(1000)
        ...
        vI2 = vI2/self.nTriggers*np.power(10.0,dLevelCorrection/10.0)
        vQ2 = vQ2/self.nTriggers*np.power(10.0,dLevelCorrection/10.0)
        vPowerMean = vPowerMean*np.power(10.0,dLevelCorrection/10.0)
