...
#plt.plot(np.arange(-lags+1,lags), autocorr2[segments*lags-lags:segments*lags+lags-1])
plt.plot(np.arange(-lags+1,lags), result)
print result.max()

#res2 = signal.convolve(sig0, sig0[::-1], mode='full')/(len(sig0)-1) #this simply takes way too long!!
